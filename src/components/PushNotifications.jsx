...
const NotificationContainer = ({ notifications, onRemove, onMarkRead }) => {
  const getIcon = (icon) => {
    switch (icon) {
      ...
      case 'trip': return <HiOutlineCheckCircle className="w-5 h-5 text-amber-500" />;
      case 'completed': return <HiOutlineCheckCircle className="w-5 h-5 text-green-500" />;
      case 'cancelled': return <HiOutlineExclamationTriangle className="w-5 h-5 text-red-500" />;
      ...
      {notifications.map((notification) => (
        ...
        >
          <div className="flex items-start gap-3">
            ...
            >
              <HiOutlineXMark className="w-4 h-4 text-sand-400" />
            </button>
          </div>
        </div>
      ))}
...
export const NotificationPermissionBanner = () => {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-90 bg-amber-500 text-twende-text px-4 py-3">
