...
import { HiOutlineMapPin, HiOutlinePhone, HiOutlineXMark, HiOutlineExclamationTriangle } from 'react-icons/hi2';

const EMERGENCY_CONTACTS = {
  police: { number: '112', name: 'Police', icon: FiShield },
  ambulance: { number: '114', name: 'Ambulance', icon: HiOutlinePhone },
  fire: { number: '115', name: 'Fire Department', icon: HiOutlineExclamationTriangle },
};
  ...
  if (alertSent) {
    return (
      <div className="fixed bottom-4 right-4 z-50">
        <div className="bg-flame-500 text-twende-text px-6 py-4 rounded-2xl shadow-lg animate-pulse">
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-20 right-4 z-40 bg-flame-500 hover:bg-flame-600 text-twende-text p-4 rounded-full shadow-lg transition-transform hover:scale-110 active:scale-95"
        aria-label="Emergency SOS"
      >
        <span className="text-xs font-bold block">SOS</span>
        <HiOutlineExclamationTriangle className="w-5 h-5 mx-auto" />
      </button>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in duration-200">
            {/* Header */}
            <div className="bg-flame-500 text-twende-text p-6">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <HiOutlinePhone className="w-8 h-8 text-blue-600 mx-auto mb-2" />
              ...
              <button
                onClick={handleSOS}
                disabled={isSending}
                className="w-full bg-flame-500 hover:bg-flame-600 disabled:opacity-50 text-twende-text font-bold py-4 rounded-xl shadow-lg transition flex items-center justify-center gap-3"
              >
                {isSending ? (
                  <>
